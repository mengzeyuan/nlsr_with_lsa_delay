                                ...
                                bool timedProcessing/*=false*/)
{
  ...
  DigestConstPtr rootDigest;   //得到当前state digest
  {
  ...
  if (*rootDigest == *digest)   //与当前state digest相同
    {
      _LOG_DEBUG_ID ("processSyncInterest (): Same state. Adding to syncInterestTable");
      m_syncInterestTable.insert (digest, name.toUri(), false);
      return;
    }

  //与当前state digest不同，能在log记录中找到相同的state digest
  DiffStateContainer::iterator stateInDiffLog = m_log.find (digest);

  if (stateInDiffLog != m_log.end ())    //在log记录中找到所收到的state digest,证明自己的state digest无需更新，需要满足对方缺少的部分
  {
    DiffStateConstPtr stateDiff = (*stateInDiffLog)->diff ();

    sendSyncData (name, digest, stateDiff);    //发送sync data，回复所拥有最新的state digest
    return;
  }

  //与当前state digest不同，在log记录中找不到相同的state digest，需发送SyncRecoveryInterest
  if (!timedProcessing)  //检查计时器是否到时
    {
      bool exists = m_syncInterestTable.insert (digest, name.toUri(), true);   //将syncInterest插入syncInterestTable,如果该interest已存在，那么更新计时器
      if (exists) // somebody else replied, so restart random-game timer       //syncInterestTable：保存未回复的syncInterest
        {
  ...
  else
    {
      ...
      _LOG_DEBUG_ID("sendSyncRecoveryInterests:"<<digest);    //ymz
      m_recoveryRetransmissionInterval = m_defaultRecoveryRetransmitInterval;
  ...
  try
    {
      ...
      m_syncInterestTable.remove (name.toUri()); // Remove satisfied interest from syncInterestTable
      ...
      vector<MissingDataInfo> v;   //用v来存储sync data里的LSA内容名？？
  ...
  if ((diffLog != 0 && diffLog->getLeaves ().size () > 0) ||
      ownInterestSatisfied)
    {
      ...
      EventId eventId = m_scheduler.scheduleEvent (after,
                                                   bind (&SyncLogic::sendSyncInterest, this));  //发送更新后的sync interest
...
SyncLogic::processSyncRecoveryInterest (const Name &name, DigestConstPtr digest)
{
  ...
  if (stateInDiffLog == m_log.end ())   //在log中找不到收到的state digest，不能回复syncdata
    {
      ...
      while (m_syncInterestTable.size () > 0)
        {
          ...
          if (!interest.m_unknown)
            {
              _LOG_DEBUG_ID ("satisfyPendingSyncInterests");  //ymz
...
void
SyncLogic::addLocalNames (const Name &prefix, uint64_t session, uint64_t seq)  //发布新内容
{
  DiffStatePtr diff;
  {
  ...
  satisfyPendingSyncInterests (diff);   //state digest变化，满足sync interest table中的sync interest
}

void
SyncLogic::remove(const Name &prefix)   //移除某内容
{
  DiffStatePtr diff;
  {
    NameInfoConstPtr info = StdNameInfo::FindOrCreate(prefix.toUri());
...
{
  _LOG_DEBUG_ID ("sendSyncData");   //ymz
  _LOG_DEBUG_ID (">> D " << name);
