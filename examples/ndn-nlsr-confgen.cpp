...
bool ProcessCommonConfig() {
  ...
  std::string common_conf = "src/ndnSIM/examples/ndn-nlsr-conf/static_nlsr.conf";
  bool ret = true;
...
ProcessBriteTopology(std::string confFile, std::string rtType)
{
  ...
  while (!topgen.eof ()) {
    ...
    std::string helloInterval = "30";  //ZhangYu 2019-6-2 change frome 60 to 1
