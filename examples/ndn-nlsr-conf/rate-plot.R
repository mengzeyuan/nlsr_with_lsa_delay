...
#########################
data = read.table("5-nlsr-l3-rate-trace.txt", header=T)   ###########################################
data$Node = factor(data$Node)
data$FaceId <- factor(data$FaceId)
data$Kilobits <- data$Kilobytes * 1 ###############################################################   *8
...
png("src/ndnSIM/docs/source/_static/root-rates2.png", width=500, height=250)#######################################
print(g.root)
retval <- dev.off()
